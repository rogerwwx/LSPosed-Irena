...
#include "api/zygisk.h"

#include "ipc_bridge.h"

using namespace std::literals::string_view_literals;

namespace lspd::native::module {
...
constexpr uid_t kManagerUid = INJECTED_AID;  // com.android.shell
...
/**
 * @class LspdModule
 * @brief The core implementation of the Zygisk module for the LSPosed framework.
 *
 ...
 * - lspd::native::Context: To gain the core injection capabilities (DEX loading, ART hooking)
 ...
 */
class LspdModule : public zygisk::ModuleBase, public lspd::native::Context {
public:
...
// =========================================================================================
// Implementation of LspdModule
// =========================================================================================

void LspdModule::LoadDex(JNIEnv *env, PreloadedDex &&dex) {
...
void LspdModule::SetupEntryClass(JNIEnv *env) {
...
void LspdModule::onLoad(zygisk::Api *api, JNIEnv *env) {
    this->api_ = api;
    this->env_ = env;
    ...
    ConfigImpl::Init();
    LOGD("LSPosed-Irena Zygisk module loaded");
}

void LspdModule::preAppSpecialize(zygisk::AppSpecializeArgs *args) {
...
void LspdModule::postAppSpecialize(const zygisk::AppSpecializeArgs *args) {
    if (!should_inject_) {
...
void LspdModule::preServerSpecialize(zygisk::ServerSpecializeArgs *args) {
...
void LspdModule::postServerSpecialize(const zygisk::ServerSpecializeArgs *args) {
    if (!should_inject_) {
        SetAllowUnload(true);
        return;
    }
    ...
    this->FindAndCall(env_, "forkCommon",
                      ...
                      system_name.get(), nullptr, manager_binder.get());

    LOGI("Injected LSPosed framework into system_server.");
    SetAllowUnload(false);  // We are injected, PREVENT module unloading.
}

void LspdModule::SetAllowUnload(bool unload) {
    if (api_ && unload) {
...
// =========================================================================================
REGISTER_ZYGISK_MODULE(lspd::native::module::LspdModule);