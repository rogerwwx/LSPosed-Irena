...
// These are the "secret handshakes" used to communicate with the daemon service.
...
// Transaction codes for specific actions. Neither side writes an interface
// token, so no descriptor string ever appears in the parcel. The bridge code
// must stay in sync with BridgeService.java in this module and in the daemon.
constexpr jint kBridgeTransactionCode = ('_' << 24) | ('I' << 16) | ('R' << 8) | 'E';
...
std::map<std::string, std::string> IPCBridge::FetchObfuscationMap(JNIEnv *env, jobject binder) {
    ...
    if (size < 0 || (size % 2 != 0)) {
        LOGE("Invalid size for obfuscation map received: {}", size);
        return result_map;
    }
