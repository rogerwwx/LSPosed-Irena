...
public class BridgeService {
    // Must stay in sync with kBridgeTransactionCode in the native IPCBridge.
    // No side writes an interface token, so no "LSPosed" descriptor string ever
    // lands in a binder parcel for the framework to leak through heap scans.
    private static final int TRANSACTION_CODE = ('_' << 24) | ('I' << 16) | ('R' << 8) | 'E';
