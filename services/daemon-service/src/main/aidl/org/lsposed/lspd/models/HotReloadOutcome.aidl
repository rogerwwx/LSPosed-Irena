 ...
 *
 * <p>Three fields rather than one status, because the libxposed service API's encoding is lossy
 * in two places and both losses are ones a module app cannot recover from.</p>
 */
parcelable HotReloadOutcome {
     ...
     * exactly one kind of failure. {@code HotReloadResult} reserves FAILED-with-a-null-message for
     * a module refusal: it means {@code onHotReloading} returned false, and no other failure may
     * present that way.
     */
    String message;
